...
@section('content')
    <div class="space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-gradient-to-r from-green-50 to-green-100 rounded-lg p-4">
                        ...
                        <div class="w-full bg-green-200 rounded-full h-2">
                            <div class="bg-green-500 h-2 rounded-full progress-bar"
                                style="width: {{ $totalBerkas > 0 ? ($berkasAktif / $totalBerkas) * 100 : 0 }}%"></div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gradient-to-r from-yellow-50 to-yellow-100 rounded-lg p-4">
                        ...
                        <div class="w-full bg-yellow-200 rounded-full h-2">
                            <div class="bg-yellow-500 h-2 rounded-full progress-bar"
                                style="width: {{ $totalBerkas > 0 ? ($berkasInaktif / $totalBerkas) * 100 : 0 }}%"></div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg p-4">
                        ...
                        <div class="w-full bg-blue-200 rounded-full h-2">
                            <div class="bg-blue-500 h-2 rounded-full progress-bar"
                                style="width: {{ $totalBerkas > 0 ? ($berkasPermanen / $totalBerkas) * 100 : 0 }}%"></div>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <div class="flex-1">
                                ...
                                <div class="w-full bg-gray-100 rounded-full h-2">
                                    <div class="bg-gradient-to-r from-green-400 to-green-600 h-2 rounded-full progress-bar"
                                        style="width: {{ $klasifikasiStats->max('count') > 0 ? ($stat['count'] / $klasifikasiStats->max('count')) * 100 : 0 }}%">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    @forelse($unitKerjaStats as $unit)
                        <div class="flex items-center">
                            <div class="flex-1">
                                <div class="flex items-center justify-between mb-1">
                                    <span
                                        class="text-sm font-medium text-gray-700">{{ Str::limit($unit->unit_kerja, 30) }}</span>
                                ...
                                <div class="w-full bg-gray-100 rounded-full h-2">
                                    <div class="bg-gradient-to-r from-blue-400 to-blue-600 h-2 rounded-full progress-bar"
                                        style="width: {{ $unitKerjaStats->max('total') > 0 ? ($unit->total / $unitKerjaStats->max('total')) * 100 : 0 }}%">
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="p-4">
                    <div class="space-y-3">
                        @forelse($berkasLatest as $berkas)
                            <a href="{{ route('berkas-arsip.show', $berkas) }}"
                                ...
                                <div class="flex items-start">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-xs text-gray-500 truncate">{{ Str::limit($berkas->uraian_berkas, 35) }}
                                        </p>
                                        <div class="flex items-center mt-1 text-xs text-gray-400">
                                            <span>{{ $berkas->created_at->diffForHumans() }}</span>
                                            <span class="mx-2">•</span>
                                            <span
                                                class="px-2 py-0.5 rounded text-xs {{ $berkas->status_arsip == 'Aktif' ? 'bg-green-100 text-green-700' : ($berkas->status_arsip == 'Inaktif' ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-100 text-blue-700') }}">
                                                {{ $berkas->status_arsip }}
                                            </span>
                                        </div>
                                    </div>
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                <div class="px-6 py-4 border-b-2 flex justify-between items-center"
                    style="background-color: rgba(59, 130, 246, 0.05); border-color: #3b82f6;">
                ...
                <div class="bg-white rounded-xl shadow-lg p-6">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <a href="{{ route('berkas-arsip.create') }}"
                            ...
                            <div
                                class="w-12 h-12 bg-green-500 rounded-xl flex items-center justify-center mb-2 group-hover:scale-110 transition-transform">
                        ...
                        <a href="{{ route('klasifikasi-arsip.create') }}"
                            ...
                            <div
                                class="w-12 h-12 bg-blue-500 rounded-xl flex items-center justify-center mb-2 group-hover:scale-110 transition-transform">
                                <i class="fas fa-plus-circle text-white text-xl"></i>
                            </div>
                            <span class="text-xs font-medium text-gray-700 text-center">Klasifikasi Baru</span>
                        </a>
                        <a href="{{ route('lokasi-arsip.create') }}"
                            class="flex flex-col items-center p-4 bg-orange-50 rounded-xl hover:bg-orange-100 transition-colors group">
                            <div
                                class="w-12 h-12 bg-orange-500 rounded-xl flex items-center justify-center mb-2 group-hover:scale-110 transition-transform">
                        ...
                        <a href="{{ route('berkas-arsip.index') }}"
                            ...
                            <div
                                class="w-12 h-12 bg-purple-500 rounded-xl flex items-center justify-center mb-2 group-hover:scale-110 transition-transform">
                    ...
                    <div class="space-y-3">
                        @forelse($fileTypes as $type)
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    <i
                                        class="fas {{ $type->tipe_file == 'pdf' ? 'fa-file-pdf text-red-500' : 'fa-file-image text-blue-500' }} mr-2"></i>

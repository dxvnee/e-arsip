...
class LaporanController extends Controller
{
    ...
    public function disposisi(Request $request)
    {
        ...
        $disposisi = Disposisi::with(['arsip', 'dariUser', 'kepadaUser'])
            ->whereBetween('created_at', [$request->tanggal_mulai, $request->tanggal_akhir])
            ->get();
        
        $statsStatus = Disposisi::select('status', DB::raw('count(*) as total'))
            ->whereBetween('created_at', [$request->tanggal_mulai, $request->tanggal_akhir])
            ->groupBy('status')
            ->get();
        
        return view('laporan.disposisi', compact('disposisi', 'statsStatus', 'request'));
    }
}
